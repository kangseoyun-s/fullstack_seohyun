...
| 역할      | 주요 테이블                            | 핵심 컬럼                       | 담당 CRUD 기능     | 담당자 |
| ------- | --------------------------------- | --------------------------- | -------------- | ---- |
| ① 유저 관리 | USER_TB                           | user_id, email, preference  | 회원가입, 로그인, 프로필 | 희재님 |
| ② 음식 관리 | FOOD_TB                           | food_id, name, kcal, recipe | 음식 등록, 수정, 삭제  | 정민님 |
| ③ 재료 관리 | INGREDIENT_TB, FOOD_INGREDIENT_TB | ingredient_id, name, amount | 재료 등록, 매핑      |서현님 |
| ④ 추천·검색 | RECOMMEND_TB, SEARCH_LOG_TB       | rec_id, keyword, type       | 추천, 검색 히스토리    | 영민님 |
| ⑤ 커뮤니티  | COMMUNITY_TB, REVIEW_TB           | post_id, review_id          | 게시글/리뷰 CRUD    | 상현님 |
