...
//		arr [1][0] = data++; arr[1][1]=data++; arr[1][2]=data++;
		int data=1;
		{for(int ch =0; ch<arr.length; ch++) {for(int kan=0; kan<arr[ch].length; kan++){
			arr[ch][kan] = data++;}}
		for(int kan=0; kan<arr[0].length; kan++){arr[0][kan]=data++;}
		for(int kan=0; kan<arr[0].length; kan++){arr[1][kan]=data++;}
		
		for(int ch =0; ch<arr.length; ch++) {//아파트.층 (몇 층이니?)
			for(int kan=0; kan<arr[ch].length; kan++) {// 층은 몇 칸이니?
